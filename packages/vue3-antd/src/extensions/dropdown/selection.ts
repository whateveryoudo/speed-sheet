...
export function selectionHasDropdowns(sheet: Sheet): boolean {
  let found = false
  eachCellInSelection(sheet, (r, c) => {
    if (sheet.state.getDataVerification(r, c)?.type === 'dropdown') found = true
  })
  return found
}

/** 删除选区内全部下拉列表，返回是否删除了至少一个 */
export function removeDropdownsInSelection(sheet: Sheet): boolean {
  const targets: Array<{ r: number; c: number }> = []
  eachCellInSelection(sheet, (r, c) => {
    if (sheet.state.getDataVerification(r, c)?.type === 'dropdown') {
      targets.push({ r, c })
    }
  })
